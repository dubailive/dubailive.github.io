...
function clock(options) {
    ...
    var getTimes = function () {
		...
		funcs = {
			...
			if (rhr === 12) {
			    return 'ITS LUNCH TIME <br />Bon appetite !';
			}
			if (rhr === 0) {
			    return "ITS MIDNIGHT <br />go to sleep !";
			}
						if (rhr > 0 && rhr < 3) {
			    return 'STILL AWAKE ?<br />count some sheep.';
			}
						if (rhr > 2 && rhr < 6) {
			    return 'too early to get up<br />SLEEP ON !';
			}
						if (rhr > 5 && rhr < 7) {
			    return 'NO MORE NIGHT<br />but also not really day !';
			}
			if (rhr > 6 && rhr < 10) {
			    return 'GOOD MORNING SUNSHINE<br />have a nice day !';
			}
			if (rhr > 9 && rhr < 12) {
			    return 'TAKE A BREAK<br />time for coffee!';
			}
			if (rhr > 12 && rhr < 18) {
			    return 'GOOD AFTERNOON <br /> Its again Coffee Time';
			}
			if (rhr > 17 && rhr < 20) {
			    return 'DINNERTIME !<br />getting hungry yet';
			}
						if (rhr > 19 && rhr < 21) {
			    return 'DAY IS COMING TO A CLOSE<br />have a nice evening !';
			}
			if (rhr > 20 && rhr < 24) {
			    return 'TIME TO RELAX !<br />be comfortable';
			}
		    },
		    dateplus: function () {
